(() => {
  ...
  async function logsPage() {
    const channels=await api('/api/channels');app.innerHTML=`<div class="page">${pageHead('REQUESTS','请求日志','最近的 Claude 路由请求与内存失败记录。','<button class="btn danger" id="clear-fail">清空失败记录</button>')}<div class="toolbar"><select class="compact-select" id="log-channel"><option value="">全部渠道</option>${channels.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select><button class="btn" id="refresh-logs">刷新</button></div><section class="panel"><div class="table-wrap"><table><thead><tr><th>时间</th><th>渠道</th><th>模型</th><th>输入</th><th>输出</th><th>缓存 写 / 读</th><th>耗时</th><th>状态</th></tr></thead><tbody id="log-body"></tbody></table></div><div id="log-empty"></div></section><div class="section-title"><h2>失败记录</h2><span>服务重启后清空</span></div><section class="panel"><div class="panel-body stack" id="failures"></div></section></div>`;
    const load=async()=>{const id=formValue('log-channel');const [logs,failures]=await Promise.all([api(`/api/logs?limit=100${id?'&channel_id='+id:''}`),api('/api/failures')]);document.getElementById('log-body').innerHTML=logs.map(r=>`<tr><td>${date(r.request_at)}</td><td>${esc(r.channel_name||'—')}</td><td class="code">${esc(r.model||'—')}</td><td>${fmt(r.input_tokens)}</td><td>${fmt(r.output_tokens)}</td><td>${fmt(r.cache_creation_tokens)} / ${fmt(r.cache_read_tokens)}</td><td>${r.duration_ms||0}ms</td><td><span class="status ${Number(r.status)<400?'on':'warn'}">${r.status}</span></td></tr>`).join('');document.getElementById('log-empty').innerHTML=logs.length?'':'<div class="empty"><strong>暂无请求</strong>新的转发记录会显示在这里。</div>';document.getElementById('failures').innerHTML=failures.length?failures.map(f=>`<div><div class="row-title">${esc(f.channel||'未知渠道')} · ${esc(f.error_type||'请求失败')}</div><div class="row-sub">${date(f.ts)} · ${esc(f.upstream_body||f.error_repr||'')}</div></div>`).join(''):'<div class="empty"><strong>没有失败记录</strong>当前没有捕获到上游错误。</div>'};document.getElementById('log-channel').onchange=load;document.getElementById('refresh-logs').onclick=load;document.getElementById('clear-fail').onclick=async()=>{await api('/api/failures',{method:'DELETE'});toast('失败记录已清空');load()};await load();
  }
