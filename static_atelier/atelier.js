(() => {
  ...
  function setupCodexAuth(id){
    ...
    document.getElementById('sa-auth-complete').onclick=async()=>{if(!session){toast('请先连接 OpenAI',true);return}const callbackUrl=formValue('sa-callback-url').trim();if(!callbackUrl){toast('请粘贴完整回调 URL',true);return}status.textContent='正在校验回调并保存凭据…';try{await api(`/api/codex/accounts/${id}/authcode/complete`,{method:'POST',body:JSON.stringify({login_session_id:session.login_session_id,callback_url:callbackUrl})});status.textContent='浏览器回调授权成功';toast('Codex 授权成功');session=null}catch(e){status.textContent=e.message;toast(e.message,true)}};
