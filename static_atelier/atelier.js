(() => {
  ...
  function codexAccountSummary(account, usage=account?.usage||{}) {
    ...
    return `<strong>${account?.authenticated?'已授权':'未授权'}</strong><span>${esc(plan)}${account?.email?' · '+esc(account.email):''}</span>`;
  }
  function codexModelSummary(raw) {
  ...
  function subscriptionAuthPanel(kind,id) {
    if(!id)return '';
    if(kind!=='codex')return '<button class="btn dark" id="sa-auth">验证并登录</button><div class="notice" id="sa-auth-state"></div>';
    return `<div class="stack"><label class="field"><span>鉴权方式</span><input id="sa-auth-mode" type="hidden" value="callback"><div class="segmented" id="sa-auth-segments"><button class="segment active" data-value="callback" type="button">浏览器回调</button><button class="segment" data-value="device" type="button">设备代码</button></div></label><div class="stack" id="sa-callback-panel"><button class="btn dark" id="sa-auth-browser" type="button">连接 OpenAI</button><div class="stack" id="sa-callback-fields" hidden><label class="field"><span>回调 URL</span><textarea id="sa-callback-url" rows="3" spellcheck="false" placeholder="http://localhost:1455/auth/callback?code=...&state=..."></textarea><small>登录完成后页面无法打开是正常的，粘贴浏览器地址栏中的完整 URL。</small></label><button class="btn primary" id="sa-auth-complete" type="button">完成授权</button></div></div><button class="btn dark" id="sa-auth-device" type="button" hidden>开始设备代码授权</button><div class="notice" id="sa-auth-state"></div></div>`;
  }
  async function subscriptionAccountDrawer(kind,account) {
    const meta=subscriptionMeta[kind];const id=account?.id;
    openDrawer(`<div class="drawer-top"><div><p class="eyebrow">${id?'ACCOUNT':'NEW ACCOUNT'}</p><h2>${id?esc(account.name):`添加 ${meta.title}`}</h2></div><button class="icon-btn" data-close title="关闭">×</button></div><div class="drawer-body stack"><label class="field"><span>账号名称</span><input id="sa-name" value="${esc(account?.name||meta.title)}"></label><label class="field"><span>账号代理</span><input id="sa-proxy" value="${esc(account?.proxy_url||'')}" placeholder="http://user:pass@host:port"><small>留空为直连。</small></label><div class="inline-actions"><button class="btn" id="sa-test-proxy" type="button">测试代理</button><small id="sa-proxy-state"></small></div>${kind==='claudecode'?`<label class="field"><span>Claude sessionKey</span><input id="sa-credential" type="password" autocomplete="off" placeholder="${id?'留空保留当前 sessionKey':''}"></label>`:''}${id?`${kind==='codex'?`<div class="notice account-summary" id="sa-account-summary">${codexAccountSummary(account)}</div>`:''}<div class="toggle-row"><div class="toggle-copy"><strong>启用账号</strong></div><label class="switch"><input id="sa-active" type="checkbox" ${Number(account.is_active)?'checked':''}><span></span></label></div><div class="panel-body stack" id="sa-usage-panel">${usageRows(account.usage)}</div>${kind==='codex'?`<label class="field"><span>账号可用模型</span><div id="sa-model-panel">${codexModelSummary(account.models)}</div></label>`:''}`:''}<div class="drawer-actions">${id?'<button class="btn danger" id="sa-delete">删除</button>':'<span></span>'}<div class="inline-actions">${id?`<button class="btn" id="sa-usage">刷新用量</button>${kind==='codex'?'<button class="btn" id="sa-models">拉取模型</button>':''}`:''}<button class="btn primary" id="sa-save">${id?'保存':'创建'}</button></div></div>${subscriptionAuthPanel(kind,id)}</div>`);
    document.getElementById('sa-proxy').closest('.field').querySelector('small').textContent='此代理只属于当前账号；支持 URL 或 host:port:user:password，留空直连。';
    drawer.querySelectorAll('[data-close]').forEach(x=>x.onclick=closeDrawer);
    ...
    if(!id)return;document.getElementById('sa-delete').onclick=async()=>{if(!confirm('确定删除账号？'))return;await api(`/api/${kind}/accounts/${id}`,{method:'DELETE'});closeDrawer();toast('账号已删除');await subscriptionPage(kind)};document.getElementById('sa-usage').onclick=()=>actionStatus('sa-auth-state',async()=>{const usage=await api(`/api/${kind}/accounts/${id}/usage`,{method:'POST'});document.getElementById('sa-usage-panel').innerHTML=usageRows(usage);if(kind==='codex')document.getElementById('sa-account-summary').innerHTML=codexAccountSummary(account,usage);await subscriptionPage(kind);return `用量已刷新${usage?.plan_type?' · '+usage.plan_type:''}`});if(kind==='codex'){document.getElementById('sa-models').onclick=()=>actionStatus('sa-auth-state',async()=>{const d=await api(`/api/codex/accounts/${id}/models`,{method:'POST'});document.getElementById('sa-model-panel').innerHTML=codexModelSummary(d.models);await subscriptionPage(kind);return `已拉取并保存 ${d.models.length} 个模型`});setupCodexAuth(id)}else document.getElementById('sa-auth').onclick=()=>actionStatus('sa-auth-state',async()=>{await api(`/api/claudecode/accounts/${id}/authenticate`,{method:'POST'});return '验证成功'});
  }
  function setupCodexAuth(id){
    const status=document.getElementById('sa-auth-state');const callbackPanel=document.getElementById('sa-callback-panel');const deviceButton=document.getElementById('sa-auth-device');let session=null;
    const selectMode=mode=>{document.getElementById('sa-auth-mode').value=mode;document.querySelectorAll('#sa-auth-segments .segment').forEach(x=>x.classList.toggle('active',x.dataset.value===mode));callbackPanel.hidden=mode!=='callback';deviceButton.hidden=mode!=='device';status.textContent=''};
    document.querySelectorAll('#sa-auth-segments .segment').forEach(x=>x.onclick=()=>selectMode(x.dataset.value));
    document.getElementById('sa-auth-browser').onclick=async()=>{const popup=window.open('about:blank','_blank');if(popup)popup.opener=null;status.textContent='正在创建安全登录会话…';try{session=await api(`/api/codex/accounts/${id}/authcode/start`,{method:'POST'});document.getElementById('sa-callback-fields').hidden=false;status.innerHTML=`授权窗口已打开；会话将在 ${Math.round(Number(session.expires_in||600)/60)} 分钟后过期。未弹出时可 <a href="${esc(session.authorize_url)}" target="_blank" rel="noopener">点击这里</a>。`;if(popup)popup.location.replace(session.authorize_url)}catch(e){if(popup)popup.close();status.textContent=e.message;toast(e.message,true)}};
    document.getElementById('sa-auth-complete').onclick=async()=>{if(!session){toast('请先连接 OpenAI',true);return}const callbackUrl=formValue('sa-callback-url').trim();if(!callbackUrl){toast('请粘贴完整回调 URL',true);return}status.textContent='正在校验回调并保存凭据…';try{const result=await api(`/api/codex/accounts/${id}/authcode/complete`,{method:'POST',body:JSON.stringify({login_session_id:session.login_session_id,callback_url:callbackUrl})});session=null;await subscriptionPage('codex');await subscriptionAccountDrawer('codex',result.account);document.getElementById('sa-auth-state').textContent='浏览器回调授权成功，账号凭据已保存';toast('Codex 授权成功')}catch(e){status.textContent=e.message;toast(e.message,true)}};
    deviceButton.onclick=()=>codexDeviceAuth(id);
  }
  async function codexDeviceAuth(id){const state=document.getElementById('sa-auth-state');try{const d=await api(`/api/codex/accounts/${id}/device/start`,{method:'POST'});state.innerHTML=`打开 <a href="${esc(d.verification_url)}" target="_blank" rel="noopener">${esc(d.verification_url)}</a>，输入代码 <strong>${esc(d.user_code)}</strong>。`;const poll=async()=>{const r=await api(`/api/codex/accounts/${id}/device/poll`,{method:'POST',body:JSON.stringify({device_auth_id:d.device_auth_id,user_code:d.user_code})});if(r.status==='ready'){await subscriptionPage('codex');await subscriptionAccountDrawer('codex',r.account);document.getElementById('sa-auth-state').textContent='设备代码授权成功，账号凭据已保存';toast('Codex 授权成功');return}setTimeout(poll,Math.max(1000,Number(d.interval||5)*1000))};setTimeout(poll,Math.max(1000,Number(d.interval||5)*1000))}catch(e){state.textContent=e.message;toast(e.message,true)}}
  ...
  async function settingsPage() {
    ...
    app.innerHTML=`<div class="page">${pageHead('PREFERENCES','设置','管理客户端访问密钥、管理员密码和各类转发地址。')}<div class="work-grid"><section class="panel"><div class="panel-head"><h3>连接与安全</h3></div><div class="panel-body stack"><label class="field"><span>客户端访问密钥</span><input id="s-access" value="${esc(access.value||'')}"><small>Claude、OpenAI、Codex 和 Claude Code 路由均使用此密钥。</small></label><button class="btn primary" data-save="access_key">保存访问密钥</button><label class="field"><span>管理员密码</span><input id="s-password" type="password" placeholder="输入新密码"><small>修改后在下次登录时生效。</small></label><button class="btn" data-save="admin_password">更新管理员密码</button></div></section><aside class="panel"><div class="panel-head"><h3>客户端地址</h3><span>API ROUTES</span></div><div class="panel-body stack"><div class="field"><span>Claude 渠道根地址</span><input readonly value="${esc(location.origin+'/\u003c渠道名\u003e/v1')}"></div><div class="field"><span>OpenAI API</span><input readonly value="${esc(location.origin+'/gpt/v1')}"></div><div class="field"><span>Codex 订阅</span><input readonly value="${esc(location.origin+'/codex/v1')}"></div><div class="field"><span>Claude Code 订阅</span><input readonly value="${esc(location.origin+'/claudecode/v1')}"></div></div></aside></div></div>`;
